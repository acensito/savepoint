...
- `Tests\Feature\Web\GameControllerTest`: alta y edición de juegos con subida/reemplazo de carátula real, validación, aviso de EAN duplicado (con y sin confirmar), `GamePolicy` aplicada en las rutas web, la ficha de detalle, la edición rápida (conservación/estado/en venta) por AJAX y por formulario normal, el filtro "en venta", marcar un juego como vendido (validación, envío a la papelera, `GamePolicy`) y que la papelera excluye los juegos vendidos, el fragmento que devuelve `index()` para peticiones AJAX, la papelera (listar/restaurar/eliminar definitivamente, buscador/filtro propio, con scoping por usuario), el orden/paginación/región/edición por defecto de Ajustes (aplicados solo cuando la URL o el formulario no traen un valor explícito), el autoasignado de fondo desde IGDB al dar de alta con ese ajuste activo, y la búsqueda de carátula/EAN en CEX tanto para un juego ya guardado (por su EAN o su título, o por una búsqueda manual) como para el alta (sin resultados ni llamada a CEX sin `q`, requiere sesión iniciada).
...
- **Bug: el icono de formato (digital/CIAB) no sale en el listado de la colección**, siempre se ve el de físico (el disco) sin importar el formato real de la edición. Causa probable: `GameController::index()` (`app/Http/Controllers/Web/GameController.php:97`) carga la relación `edition` limitada a `id,name` (`'edition:id,name'`), sin `format` — con esa columna sin traer, `Edition::formatIcon()` no encuentra `FORMATS[$this->format]` y cae siempre al icono de físico por el `??` de fallback. Arreglo probable: añadir `format` a esa selección (`'edition:id,name,format'`).
- **Bug: en móvil, el gesto de "atrás" justo después de dar de alta un juego puede reenviar el formulario y duplicarlo.** El formulario de alta (`games/_form.blade.php`) no deshabilita el botón de enviar al hacer submit, así que si el navegador restaura la página del formulario desde el bfcache (caché de navegación atrás/adelante) tras el gesto, el botón sigue habilitado con los datos todavía rellenos y un segundo toque vuelve a enviarlo — a diferencia del resto de forms de la app con doble confirmación (`js-confirm-delete`), este no tiene ninguna guarda contra reenvíos. Arreglo probable: deshabilitar el botón de enviar en el propio `submit` del formulario (el bfcache restaura el DOM tal cual estaba al navegar fuera, así que quedaría deshabilitado también al volver) y/o escuchar `pageshow` con `event.persisted` para resetearlo explícitamente.
