...
- **Docker Engine** y **Docker Compose v2** (el comando es `docker compose`, con espacio — no la versión antigua
  `docker-compose` con guion). No hace falta instalar PHP, Node ni PostgreSQL en el host: todo corre dentro de los
  contenedores.
- **Espacio en disco**: las imágenes (PHP, Postgres, Redis, nginx) ocupan en total unos cientos de MB la primera vez
  que se construyen. Los datos crecen con tu colección — la base de datos en sí es texto, apenas pesa nada incluso con
  miles de juegos; las carátulas son lo que más espacio real ocupa (unos pocos cientos de KB cada una subida a mano,
  algo menos las descargadas de CEX/IGDB).
- **RAM y CPU**: ver "Perfiles de recursos" más abajo — el perfil por defecto ("servidor") asume una máquina con
  recursos holgados; el perfil "NAS" está pensado para hardware más ajustado (un NAS compartiendo recursos con su
  propio sistema, un mini-PC viejo...).
- Vale cualquier sistema donde corra Docker: un NAS con soporte de contenedores (Synology Container Manager, QNAP
  Container Station...), un servidor Linux, o tu propio ordenador para probarlo en local.
...
### Perfiles de recursos: servidor vs NAS

`docker-compose.yml` trae cada servicio con dos configuraciones de `mem_limit`/`cpus` ya escritas — la de "servidor"
activa por defecto, y la de "NAS" comentada justo debajo de cada una. Son techos máximos que Docker no deja superar
(protegen al resto del sistema si algo se dispara), no la RAM que la app usa de forma continua — el uso real en el día
a día es bastante más bajo que estas cifras.

| Servicio          | Servidor (por defecto) | NAS (recursos ajustados) |
|-------------------|-------------------------|---------------------------|
| `postgres`        | 512 MB / 1.5 CPU        | 384 MB / 1.0 CPU          |
| `postgres-backup` | 64 MB / 0.1 CPU         | igual                     |
| `redis`           | 256 MB / 0.5 CPU        | 160 MB / 0.5 CPU          |
| `app`             | 512 MB / 1.5 CPU        | 320 MB / 1.0 CPU          |
| `queue`           | 256 MB / 0.5 CPU        | 160 MB / 0.5 CPU          |
| `nginx`           | 64 MB / 0.25 CPU        | 32 MB / 0.25 CPU          |
| **Total aprox.**  | **~1,7 GB / 4,35 CPU**  | **~1,1 GB / 3,35 CPU**    |

Para pasar al perfil NAS, en `docker-compose.yml`: por cada servicio, comenta las dos líneas bajo "Perfil servidor" y
descomenta las que hay bajo "Perfil NAS" (`redis` tiene además una tercera línea, el propio `command`, con un
`maxmemory` más bajo para no acaparar un NAS con poca RAM). Después:

```bash
docker compose up -d --build
```

Pensado contra un caso real: un HP MicroServer N36L compartiendo RAM/CPU con OpenMediaVault y sus propios plugins.
...
El detalle de qué prueba cada clase de test, controlador a controlador, vive aparte en
[`docs/tests.md`](docs/tests.md) para no alargar este README.
