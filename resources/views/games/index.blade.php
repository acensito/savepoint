...
@section('content')
    ...
    <div class="flex items-center justify-end gap-2 mb-3">
        <div class="inline-flex items-stretch h-8 bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
            <!-- Lista, compacta y texto son variantes de la TABLA de escritorio, que
                 solo aparece a partir de xl (1280px, ver #view-list en _results.blade.php
                 y el comentario de #127 ahí mismo) — por debajo siempre se ve la
                 tarjeta, tablets incluidas. Ocultar estos botones solo por debajo de md
                 (768px) los dejaba visibles pero sin ningún efecto en el hueco intermedio
                 (md-xl, tablets normalmente): se alinea el breakpoint del botón con el
                 real, así solo queda el de estantería (que sí cambia algo visible en
                 cualquier ancho). Compacta va primero por ser la vista por defecto (ver
                 users.games_view). -->
            <button type="button" id="games-view-compact-btn" aria-label="Ver como tabla compacta" aria-pressed="false"
                class="hidden xl:flex items-center justify-center w-8 text-slate-500 hover:bg-slate-800 transition-colors">
            ...
            <button type="button" id="games-view-list-btn" aria-label="Ver como lista" aria-pressed="true"
                class="hidden xl:flex items-center justify-center w-8 text-indigo-400 hover:bg-slate-800 xl:border-l xl:border-slate-800 transition-colors">
                <x-gicon name="density_small" class="text-[15px]" />
            </button>
            <!-- #139: solo texto, sin carátulas ni espacio reservado para ellas —
                 pensada para hojear rápido por título con la colección real (1000+
                 juegos) cargando el mínimo de imágenes posible. Solo tiene sentido en
                 escritorio (la tarjeta móvil/tablet siempre lleva carátula, ver arriba). -->
            <button type="button" id="games-view-text-btn" aria-label="Ver solo texto, sin carátulas" aria-pressed="false"
                class="hidden xl:flex items-center justify-center w-8 text-slate-500 hover:bg-slate-800 xl:border-l xl:border-slate-800 transition-colors">
                <x-gicon name="description" class="text-[15px]" />
            </button>
