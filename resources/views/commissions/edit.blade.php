...
@section('content')
    ...
    <div class="max-w-2xl mx-auto py-6">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-slate-100 tracking-tight">Editar encargo</h1>
                @if($commission->isResolved())
                    <p class="text-slate-400 mt-1">
                        {{ $commission->resolvedLabel() }}. Editar esto no cambia la ficha del juego ya creado, si la hay.
                    </p>
                @endif
            </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-8">
            <form action="{{ route('web.commissions.update', $commission->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                        <label class="flex items-center gap-2 text-sm text-slate-300 border border-slate-700 rounded-lg px-4 py-2.5 cursor-pointer hover:bg-slate-800">
                            <input type="radio" name="direction" value="owed_to_me" {{ old('direction', $commission->direction) === 'owed_to_me' ? 'checked' : '' }}
                                class="text-indigo-600 focus:ring-indigo-500">
                            Me lo deben (lo recibiré)
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-300 border border-slate-700 rounded-lg px-4 py-2.5 cursor-pointer hover:bg-slate-800">
                            <input type="radio" name="direction" value="owed_by_me" {{ old('direction', $commission->direction) === 'owed_by_me' ? 'checked' : '' }}
                                class="text-indigo-600 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $commission->title) }}" required autofocus class="{{ $input }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="platform_id" id="platform_id" class="{{ $input }}">
                            ...
                            @foreach($platforms as $platform)
                                <option value="{{ $platform->id }}" {{ old('platform_id', $commission->platform_id) == $platform->id ? 'selected' : '' }}>
                                    {{ $platform->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="counterparty_name" id="counterparty_name" value="{{ old('counterparty_name', $commission->counterparty_name) }}" required
                            class="{{ $input }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $commission->price) }}" step="0.01" min="0" class="{{ $input }}">
                        @error('price') <span class="{{ $error }}">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label for="purchased_at" class="{{ $label }}">Fecha de compra</label>
                        <input type="date" name="purchased_at" id="purchased_at" value="{{ old('purchased_at', $commission->purchased_at?->format('Y-m-d')) }}" class="{{ $input }}">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3" class="{{ $input }}">{{ old('notes', $commission->notes) }}</textarea>
