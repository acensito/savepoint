...
@section('content')
    <div class="max-w-xl mx-auto py-6">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-slate-100 tracking-tight">Editar Plataforma</h1>
                <p class="text-slate-400 mt-1">{{ $platform->name }}</p>
            </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-8">
            <form action="{{ route('web.platforms.update', $platform->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
