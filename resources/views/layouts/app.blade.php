...
<div class="app-shell flex flex-col overflow-hidden">
    ...
    <header class="h-12 shrink-0 flex items-center justify-between gap-3 px-5">
        ...
        <div class="flex items-center gap-4 shrink-0">
            ...
            <button type="button"
                    class="js-theme-toggle flex items-center justify-center w-8 h-8 rounded-lg text-indigo-100 hover:bg-white/10 hover:text-white transition-colors"
            ...
            <div class="relative">
                <button type="button" id="user-menu-trigger"
                        class="js-user-menu-trigger flex items-center gap-1.5 text-sm text-indigo-100 hover:text-white transition-colors"
                ...
                <div id="user-menu"
                     class="js-user-menu hidden absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-lg py-1 text-slate-100 z-20">
                    ...
                    <a href="{{ route('web.panel.index') }}"
                       class="flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-slate-700 transition-colors {{ request()->routeIs('web.panel.*', 'web.games.import*', 'web.games.auto-identify*', 'web.games.trash') ? 'text-indigo-400' : '' }}">
                    ...
                    <form action="{{ route('web.logout') }}" method="POST">
                        @csrf
                        <button type="submit"
                                class="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-left hover:bg-slate-700 transition-colors">
        ...
        <aside id="sidebar"
            ...
            <div class="flex items-center justify-between px-3 py-2">
                ...
                <button type="button" id="sidebar-toggle"
                    ...
                    <x-gicon id="sidebar-toggle-icon" name="chevron_left"
                             class="text-[20px] transition-transform duration-200"/>
                </button>
            ...
            <div id="sidebar-footer" class="px-3 py-4 border-t border-slate-800 space-y-1">
                <a href="{{ route('web.panel.index') }}" title="Panel de control"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('web.panel.*', 'web.games.import*', 'web.games.auto-identify*', 'web.games.trash') ? 'bg-indigo-500/10 text-indigo-300' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100' }}">
...
<div id="toast-container"
     class="fixed top-16 right-4 z-50 flex flex-col gap-2 w-[calc(100%-2rem)] max-w-sm pointer-events-none"></div>
...
<dialog id="barcode-scan-dialog"
        class="rounded-xl border border-slate-800 bg-slate-900 text-slate-100 p-0 backdrop:bg-black/70 w-full max-w-sm">
    <div class="p-4">
        ...
        <video id="barcode-scan-video" class="w-full aspect-video rounded-lg bg-black object-cover" muted
               playsinline></video>

        <p id="barcode-scan-error" class="hidden mt-3 text-sm text-red-400"></p>
        <p class="mt-3 text-xs text-slate-500 text-center">Apunta la cámara al código de barras (EAN) de la caja del
            juego.</p>
    </div>
</dialog>
