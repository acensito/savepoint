...
## 2026-08-06
- **Exportación imprimible/PDF**: botón "Imprimir" en la ficha de un juego y en el listado de la colección (respeta los filtros activos, no solo la página actual de la paginación). Abre una vista independiente del layout de la app —sin sidebar/header, en blanco y negro— pensada para el diálogo de impresión nativo del navegador, que ya permite "Guardar como PDF" sin generar nada en el servidor. `GameController::index()` se refactoriza para compartir la lógica de filtrado/orden con la nueva exportación en vez de duplicarla.
